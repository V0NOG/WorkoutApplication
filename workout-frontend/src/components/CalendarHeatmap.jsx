...
export default function CalendarHeatmap({ days = [], onClick }) {
  ...
  return (
    <div className="grid grid-cols-7 gap-1 p-3 rounded-xl border border-border bg-[#0b1324]">
      {last35.map((d) => {
        let bg = "bg-white/10";         // default/unscheduled
        ...
        return (
          <button
            ...
            className={`h-5 w-5 rounded-md ${bg} hover:outline hover:outline-2 hover:outline-blue-400`}
          />
        );
      })}
    </div>
